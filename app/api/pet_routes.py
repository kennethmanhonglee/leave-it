...
from app.aws import allowed_file, delete_from_s3, get_unique_filename, upload_file_to_s3
from app.forms import EditPetForm, PetForm, PetWeightForm
    ...
    if form.validate_on_submit():
        if "image" in request.files:
            ...
            if not allowed_file(image.filename):
                return {
                    "errors": {
                        "image": "File type is not supported. Please upload a file of one of these file types: PDF, PNG, JPG, JPEG, GIF"  # noqa: E501
                    }
                }
            ...
            existing_pet = Pet.query.filter(
                Pet.name == form.data["name"], Pet.user_id == user_id
            ).first()
            if existing_pet:
...
def edit_pet(pet_id):
    ...
    if form.validate_on_submit():
        ...
        # if user had picture, and didnt upload
            ...
            new_image_url = (
                existing_pet.image_url if form.data["hasPic"] is True else None
            )
